...
ThirdOrderDistance::ThirdOrderDistance(const InterpolationThirdOrder& interpolation) :
...
{
	return (1 / 6) * data_->getMaxJerk() * pow(time - interpolation_.getTimePoint(0), 3);
}

FCL_REAL ThirdOrderDistance::getDistance_1_2(const FCL_REAL time) const
{
	return 0.5 * data_->getMaxAcceleration() * pow(time - interpolation_.getTimePoint(1), 2) + 
		interpolation_.getVelocityPoint(1) * time + 
		interpolation_.getDistancePoint(1);
}

FCL_REAL ThirdOrderDistance::getDistance_2_3(const FCL_REAL time) const
{
	return 0.5 * data_->getMaxAcceleration() * pow(time - interpolation_.getTimePoint(1), 2) -
		(1 / 6) * data_->getMaxJerk() * pow(time - interpolation_.getTimePoint(2), 3)	+ 
		0.5 * data_->getMaxJerk() * pow(interpolation_.getTimePoint(1) - interpolation_.getTimePoint(0), 2) * time + 
		interpolation_.getDistancePoint(1);
}

FCL_REAL ThirdOrderDistance::getDistance_3_4(const FCL_REAL time) const
{
	return data_->getMaxVelocity() * (time - interpolation_.getTimePoint(3) ) + 
		interpolation_.getDistancePoint(2);
}

FCL_REAL ThirdOrderDistance::getDistance_4_5(const FCL_REAL time) const
{
	return data_->getMaxVelocity() * (time - interpolation_.getTimePoint(4) ) - (1 / 6) * data_->getMaxJerk()
		* pow(time - interpolation_.getTimePoint(4), 3) + 
		interpolation_.getDistancePoint(3);
}

FCL_REAL ThirdOrderDistance::getDistance_5_6(const FCL_REAL time) const
{
	return -0.5 * data_->getMaxAcceleration() * pow(time - interpolation_.getTimePoint(5), 2) + 
		interpolation_.getVelocityPoint(2) * (time - interpolation_.getTimePoint(5) ) + 
		interpolation_.getDistancePoint(4);
}

FCL_REAL ThirdOrderDistance::getDistance_6_7(const FCL_REAL time) const
{
	return -0.5 * data_->getMaxAcceleration() * pow(time - interpolation_.getTimePoint(6), 2)
		+ (1 / 6) * data_->getMaxJerk() * pow(time - interpolation_.getTimePoint(6), 3) + 
		interpolation_.getVelocityPoint(3) * (time - interpolation_.getTimePoint(6) ) + 
		interpolation_.getDistancePoint(5);
}

FCL_REAL ThirdOrderDistance::getDistance(const FCL_REAL time) const
{
	return distance_functions_[interpolation_.getTimeUpperBound(time)](time);
}
